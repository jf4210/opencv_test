...
//			OutputDebugString(szLog);
		...
		const cv::Mat &orientations, double percentage, double delta)
	{
		...
		while (it != lines.end()) 
		{
			...
			wchar_t szLog[200] = { 0 };
			swprintf_s(szLog, L"ori1 = %f, ori2 = %f, i = %d, count = %d, consistency = %f\n", ori1, ori2, i, count, consistency);
//			OutputDebugString(szLog);
			...
			if (consistency < percentage) 
			{
				...
				cv::line(orientations, pt1, pt2, cv::Scalar(255, 0, 255));
